...
description: A simple, end-to-end guide to creating and using offchain ENS subnames with the @namespacesdk/offchain-manager SDK
---

## Overview

This guide walks you through creating and using offchain ENS subnames with the `@namespacesdk/offchain-manager` SDK. You'll be able to create a subname, and then read records.
...
```bash
npm install @namespacesdk/offchain-manager dotenv
```
...
```bash
yarn add @namespacesdk/offchain-manager dotenv
```
...
<CodeGroup>
  ...
  export const client = createOffchainClient({ mode: 'mainnet', timeout: 5000 });
...
- Update records on an existing subname using [`updateSubname`](/developer-guide/sdks/offchain-sdk/create-update-subname)
- Explore more filters with [`getFilteredSubnames`](/developer-guide/sdks/offchain-sdk/get-filtered-subnames)
- See supported chains in [`ChainName`](/developer-guide/sdks/offchain-sdk/chainname)
