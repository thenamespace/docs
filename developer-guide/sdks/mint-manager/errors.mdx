...
Three codes guard the signed mint parameters the API returns. The SDK checks the signature describes the mint you asked for before the transaction reaches a wallet, so a wrong or tampered response cannot be presented to a user as a valid-looking mint.
