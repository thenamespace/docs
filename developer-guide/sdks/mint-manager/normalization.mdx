...
This matters more than it sounds. Viem's `namehash()` normalizes nothing on its own, so without this step `Alice.eth` and `alice.eth` hash to different nodes, and an availability check can report a registered name as free.
