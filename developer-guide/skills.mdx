---
title: "Skills"
description: "Agent-ready instructions for Namespace SDKs and components."
sidebarTitle: "Skills"
icon: "scroll"
---
...
The `avatar-sdk` skill gives your agent the context it needs to manage ENS avatar and header images with the `@thenamespace/avatar` SDK. It covers SIWE v4 authentication, automatic and manual signing flows, and the Metadata Service profile media routes.
...
### Mint Manager SDK skill

The `mint-manager-sdk` skill gives your agent the context it needs to mint onchain ENS subnames with the `@thenamespace/mint-manager` SDK. It covers the check-then-mint flow, setting records at mint time, and every typed error code.

**Install in your project:**

```bash
npx skills add thenamespace/skills -s mint-manager-sdk
```

Once installed, your AI agent can:
- Check availability, price, and mint eligibility in a single `checkName` call
- Build, simulate, and submit the mint transaction on Ethereum, Base, or Optimism
- Apply text, address, and contenthash records in the same transaction as the mint
- Tell a name obstacle from a minter obstacle, so a gated wallet is not sent back to the name field
- Cap the price with `maxValue` and handle `MintManagerError` codes
