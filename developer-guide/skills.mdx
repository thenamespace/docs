...
The `mint-manager-sdk` skill is the agent instructions for minting onchain ENS subnames with `@thenamespace/mint-manager`. It includes the check-then-mint flow, setting records at mint time, and the `MintManagerError` codes.
